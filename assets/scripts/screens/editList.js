...
db.execute('SELECT id, name FROM shopping_list WHERE id = ?', {
	values: [Mobile.Application.getCurrentListId()],
	onComplete: function(rs){
		...
		db.execute('SELECT id, item, checked FROM shopping_list_item WHERE list_id = ? ORDER BY position', {
			values: [Mobile.Application.getCurrentListId()],
			onComplete: function(rs){
				...
				var doneBtn = header.addButton(MooTools.lang.get('ESL', 'done'), {
					type: 'action'
				});
				var backBtn = header.addButton(MooTools.lang.get('ESL', 'back'), {
					type: 'back'
				});
				...
				backBtn.addEvent('click', function(e){
					e.stop();
					Mobile.Application.loadScreen('viewList', 'ltr', {
				...
				doneBtn.addEvent('click', function(e){
					...
					if (done) {
						...
						db.execute('UPDATE shopping_list SET name = ? WHERE id = ?', {
							values: [nameField.get('value'), Mobile.Application.getCurrentListId()],
							onComplete: function(rs){
								items.each(function(item, i){
									...
									db.execute('INSERT INTO shopping_list_item (item, position, checked, list_id) VALUES (?, ?, ?, ?)', {
										values: [item, i, checked, Mobile.Application.getCurrentListId()],
										onComplete: function(){
