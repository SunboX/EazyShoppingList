...
db.execute('SELECT name FROM shopping_list WHERE id = ?', {
			...
			onComplete: function(rs){
				...
				items.each(function(item){
					...
					item.addEvent('click', function(e){
						...
						db.execute('UPDATE shopping_list_item SET checked = ? WHERE id = ?', {
							values: [this.retrieve('checked') ? 1 : 0, this.get('id').replace(/list-/, '')]
						});
					});
				});
				...
				deleteBtn.addEvent('click', function(e){
					db.execute('DELETE FROM shopping_list_item WHERE list_id = ?', {
						values: [Mobile.Application.getCurrentListId()],
						onComplete: function(rs){
							db.execute('DELETE FROM shopping_list WHERE id = ?', {
								values: [Mobile.Application.getCurrentListId()],
								onComplete: function(rs){
									Mobile.Application.loadScreen('main', 'ltr');
								}
